...
### Pointers and Arrays
An array's name is actually a pointer to its first element. The difference between array and a normal pointer is that an array `int x[]` actually is a constant pointer `int *const x`, so `x` can not be assigned by another array.
